'''
code copied from https://github.com/yassersouri/MuCon/blob/main/src/core/metrics/isba_code.py
'''
...
def accuracy_wo_bg(P, Y, bg_class=None, **kwargs):
    def acc_w(p, y, bg_class=None):
        ind = y != bg_class
        return np.mean(p[ind] == y[ind])

    if type(P) == list:
        # return np.mean([acc_w(P[i], Y[i], bg_class) for i in range(len(P))])
        '''
        modified: to keep consistency with acc metric of other papers
        '''
        P = np.concatenate(P, axis=0)
        Y = np.concatenate(Y, axis=0)
        # import pdb
        # pdb.set_trace()
        if type(bg_class) == list:
            for i in range(1, len(bg_class)):
                Y[Y == bg_class[i]] = bg_class[0]
            bg_class = bg_class[0]
